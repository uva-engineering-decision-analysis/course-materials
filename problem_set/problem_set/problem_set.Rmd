...
```{r compute expected payoffs, echo=FALSE}
p <- c(0.4,0.4,0.2)

Pi <- tibble(a1 = c(10,5,-1), a2=c(5,5,0), a3 = c(3,2,1))
...
<br><br>
The *expected value of information* for a forecast is defined as the expected increase in payoffs to a decision-maker when they make a decision on the basis of the forecast, compared to a default baseline. 

Formally: the expected value of information $EVI$ for forecast $\bf{\hat{p}}$, relative to baseline beliefs $\bf{p}$, is given by:

$$EVI = {\mathbb{E}\left[ \Pi(\hat{a}^*,\theta) \mid  \bf{\hat{p}}\right]} - \mathbb{E}\left[ \Pi(a^*,\theta) \mid  \bf{p}\right]$$
Less formally, $EVI$ is the answer to the question: What is the maximum amount that a risk-neutral decision-maker would be willing to spend for the forecast?[^1]

[^1]: This is an admittedly awkward construction in the present case, since the manager cannot know in advance what s/he would be willing to pay before actually receiving the forecast.

5. *For the case above, compute the expected value of information for the forecast $\bf{\hat{p}}$, relative to the default baseline information $\bf{p}$.*
...
6. *Identify the optimal decision for an agent following a maximin decision rule.*
...
7. *Identify any inadmissible actions* in the case above. (There may or may not be any.)
